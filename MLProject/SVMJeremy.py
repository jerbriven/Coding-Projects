...
# Output csv with dummy variable predictors
data.to_csv('dummyHAM_metadata.csv')
...
svm = SVC(kernel='rbf')
svm.fit(xTrain,labelsTrain)

svmPredict = svm.predict(xTest)

errors = np.not_equal(svmPredict,labelsTest)
nErrors = sum(errors)
print("Accuracy of SVM is: "+str(100*metrics.accuracy_score(labelsTest,svmPredict)))